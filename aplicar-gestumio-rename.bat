...
cd /d C:\crm-app
echo ============================================
echo   Zentric -^> Gestumio (cambio de marca)
echo ============================================
echo.
pause
...
git apply gestumio-rename.patch
if errorlevel 1 ( echo ERROR al aplicar. & pause & exit /b 1 )
echo [3/4] Commit...
git add -A
git commit -m "rebrand: Zentric -> Gestumio (marca, titulos, emails, facturacion, wordmark)"
echo [4/4] Subiendo...
git push origin main
if errorlevel 1 ( echo ERROR al subir. Entra como brachu87. & pause & exit /b 1 )
del /f /q gestumio-rename.patch 2>nul
echo.
echo ============================================
