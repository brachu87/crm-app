...
cd /d C:\crm-app
echo ============================================
echo   Zentric - Permisos completos (parte 1)
echo ============================================
echo.
pause
...
git apply zentric-permisos.patch
if errorlevel 1 ( echo ERROR al aplicar. & pause & exit /b 1 )
echo [3/4] Commit...
git add -A
git commit -m "permisos: arbol completo por modulo+funcion, fix filtro de menu por permiso ver, gate caja abrir/cerrar"
echo [4/4] Subiendo...
git push origin main
if errorlevel 1 ( echo ERROR al subir. Entra como brachu87. & pause & exit /b 1 )
del /f /q zentric-permisos.patch 2>nul
echo.
echo ============================================
